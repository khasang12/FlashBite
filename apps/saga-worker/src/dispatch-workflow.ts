...
export async function driverDispatchWorkflow(args: DispatchArgs): Promise<string> {
  ...
  for (let i = 0; i < args.maxOffers; i++) {
    ...
    if (!candidate) {
      await recordDispatchFailedActivity(args.tenantId, args.orderId, DISPATCH_FAIL_REASONS.NO_DRIVERS_AVAILABLE, args.correlationId);
      return DISPATCH_STATUS.FAILED;
    }

    tried.push(candidate);
    await recordDriverOfferedActivity(args.tenantId, args.orderId, candidate, args.correlationId);
    ...
    if (accepted.has(candidate)) {
      ...
      await recordDispatchAcceptedActivity(args.tenantId, args.orderId, candidate, args.correlationId);
      ...
      await recordOrderPickedUpActivity(args.tenantId, args.orderId, args.correlationId);
      ...
      await recordOrderDeliveredActivity(args.tenantId, args.orderId, args.correlationId);
...
async function failAfterAccept(args: DispatchArgs, driverId: string): Promise<string> {
  ...
  await recordDispatchFailedActivity(args.tenantId, args.orderId, DISPATCH_FAIL_REASONS.DELIVERY_TIMEOUT, args.correlationId);
  return DISPATCH_STATUS.FAILED;
}
